"""
trim.py — Trim the thesis_experiments dataset.
...
from model import SOLVER_NAMES
...
def select_max_per_class(labels: np.ndarray, sources: list[str], cap_set) -> np.ndarray:
    ...
    keep = []
    for label_id, name in enumerate(SOLVER_NAMES):
...
def select_paper_sizes(labels: np.ndarray, sources: list[str]) -> np.ndarray:
    ...
    for cls_idx, cap in PAPER_COUNTS.items():
        ...
        if len(idx) == 0:
            print(f"  {SOLVER_NAMES[cls_idx]:<25}: 0 samples (class empty — skipped)")
            continue
        ...
        if SUITESPARSE_ONLY:
            ...
            if len(real_idx) >= cap:
                ...
                extra   = rng.choice(synth_idx, size=min(n_synth, len(synth_idx)), replace=False)
                chosen  = np.concatenate([real_idx, extra])
            print(f"  {SOLVER_NAMES[cls_idx]:<25}: {len(idx):>6} → {len(chosen)}"
                  f"  (real={len(real_idx)}, synth used={len(chosen) - len(real_idx)})")
        else:
            chosen = idx if len(idx) <= cap else rng.choice(idx, size=cap, replace=False)
            print(f"  {SOLVER_NAMES[cls_idx]:<25}: {len(idx):>6} → {len(chosen)}")
...
def main() -> None:
            ...
            if _cap_env:
                ...
                for name in cap_set:
                    if name not in SOLVER_NAMES:
                        print(f"ERROR: '{name}' not in SOLVER_NAMES.")
                        sys.exit(1)
            else:
                cap_set = None
    ...
    counts = np.bincount(new_labels, minlength=len(SOLVER_NAMES))
    n_keep = len(new_labels)
    print(f"\n  Distribution ({n_keep} total):")
    for name, cnt in zip(SOLVER_NAMES, counts):
