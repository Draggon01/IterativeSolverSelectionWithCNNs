...
Its generation was performed on an AMD Ryzen AI 7 350 / Radeon 860 and an AMD Ryzen 5 1600 Six-Core, described in more detail in \autoref{sec:dataset} %todo get connection to my home machine back and fill the actual name.
...
The full source code, configuration files, experiment logs and LaTeX sources that were used for the thesis are publicly available on GitHub under the "Thesis-Submission" Release \footnote{\url{...}}
%todo before the last hand-in add the correct link to the github repository and make it public.
...
If a predicted solver-preconditioner pair is not the best result but comes in as a close second it won't be recognised
for the accuracy which may not be ideal when targeting the implementation of the models within a specific pipeline.
For this case two separate metrics are introduced, which also give positive feedback if the chosen solver-preconditioner
pair is only marginally slower, just the second-best choice or might be extremely failure-resistant.
...
Other metrics that need to be introduced regard the time cost within data generation,
model training and per-matrix feature and image generation.
Therefore, training time and inference costs are also logged for each experiment and can be seen in corresponding summary files.
For the data generation no specific time cost was logged, unfortunately.
...
Due to the computational cost of benchmarking all 19 solver pairs, which scales significantly with matrix size, the replication dataset is limited to matrices with up to 84{,}617 rows, compared to over one million in the original paper.
...
With this dataset the experiment was set up with a $128 \times 128$ pixel density image for the CNN branch,
following the paper, their proposed 17 features are also calculated and used.
...
They differ by their feature vector, the activation of the non-convergence penalty and model size.
...
Some solvers had especially low accuracy and F1 values, therefore an additional 6 features described in \autoref{tab:features-6} were introduced.
...
The top 6 downsampling methods by F1 value are selected from Campaign~1~(\autoref{subsec:campaign1}), as well as the sign mode.
These 7 methods are then combined into all their pairwise combinations and tested.
This results in 21 different image mode combinations that can be seen in Table~\ref{tab:results-dual-14}.
...
This results in 15 different image mode combinations each run at two image sizes, therefore 30 different models.

\section{Ensemble Evaluation}\label{sec:ensemble}

\subsection{Campaign 5: Ensemble Method}\label{subsec:ensemble-method}

The ensemble combines independently trained models by averaging their softmax probability
vectors before taking the argmax prediction.
