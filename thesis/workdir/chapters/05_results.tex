...
This chapter presents and interprets the quantitative outcomes of the experiments described in \autoref{chapter:experiments}.
...
It consistently outperforms its 13 competitors, winning the first campaign with a 64.19\% accuracy and a 63.42\% F1 value and the
second one with a 64.29\% accuracy and a 63.38\% F1 value.
...
In Campaign 1 the binary\_64 model for example has a lower accuracy and only a F1 value 0.03pp higher than the 1-NN baseline, showing that
the binary downsampling doesn't store that much valuable information when cropped to small images.
...
While investigating it showed that it stopped predicting the solver-preconditioner pairs \texttt{minres+gamg} and \texttt{fcg+gamg} which resulted in a failure rate of 0\%
meaning that the non-convergence penalty works against the model in this case, thus not working as intended in this case.

As only a certain amount of downsampling methods made sense, to introduce additional variations, RCM reordering was applied to selected modes.
...
\texttt{signed\_magnitude}, \texttt{symmetry}, and \texttt{density} (128\,px) - are therefore carried forward as the basis
for the dual-channel experiments in Campaign~3~(\autoref{subsec:campaign3}).

%todo move to conclusion probably
...
In Campaign~1 for example, \texttt{magnitude\_64} achieves the best accuracy and F1, but \texttt{signed\_magnitude\_64} achieves a lower MRT$\times$ of $1.069$ versus $1.259$, making it a better choice when runtime ratio is the priority.



%
%as these models always are beaten by other models when it comes to the failure rate and MRT which
%are way more interesting metrics for this use-case.
%
%The results also show that models with the best accuracy of the highest F1, might overall not be the most useful model implicitly,
%as in the first two Campaigns these models were always beaten by Failure Rate and MRT by other models.
%This means, if the model should focus on time improvement, it might not need the best accuracy, if the Failure Rate and MRT is better than its
%competitors it still would be a better fit for a task like that.
...
For CNN-based models the improvement is negligible, the best approach being magnitude, only shifts 0.10\,pp to 64.29\% in accuracy.
...
Combined failure rates across single-channel experiments dropped from 3.5\%--6.8\% in Campaign~1 to
3.4\%--6.0\% in Campaign~2.
...
This behaviour is reproduced in the other models as well, therefore the penalty approach needs to be examined closely as to
not only provide a shift in failure rates but realistic improvement.

\paragraph{Density collapse side-effect}
The main negative side effect in this experiment is the density collapse, where accuracy drops to 52.63\% from
previously 60.27\%.
This loss of over 10\,pp is only observed in this model and might indicate a training instability, as the drop was not reproduced in later experiments,
...
The best single-channel model, \texttt{magnitude\_64} (64.29\% accuracy, 63.38\% F1), is outperformed by the best
dual-channel 64\,px model \texttt{magnitude\_\_log\_density\_64}, which achieves 65.94\% accuracy and 65.02\% F1.
The best overall model is \texttt{magnitude\_\_rcm\_log\_density\_128} with 66.56\% accuracy, a 2.27\,pp improvement
over the best single-channel approach.

The magnitude of the gain depends strongly on which two channels are chosen; when their information encodes different properties
they excel the most.
...
dual-channel pairs, confirming that magnitude encodes discriminative information essential for solver selection.
...
The dual-channel experiments are summarised in Table~\ref{tab:results-dual-14} (Campaign~3, 14 features, 64\,px)
and Table~\ref{tab:results-dual-20} (Campaign~4, 20 features, 64\,px and 128\,px).
...
The dominant finding over these two Campaigns is that \texttt{magnitude} is the most accurate channel also here.
Every top-performing pair across both campaigns includes it as one channel.
Replacing \texttt{magnitude} with any other mode as the primary channel reduces accuracy by 2--3\,pp.
...
128\,px consistently outperforms 64\,px by $+0.5$--$+1.3$\,pp across all mode pairs,
...
Dual-channel models outperform the best single-channel model by $+2.3$\,pp in accuracy
...
In the dual-channel experiment with 64 and 128\,px images, shown in \autoref{tab:results-dual-20}, the increase in resolution
brings a consistent improvement of 0.5--1.3\,pp in accuracy and F1 values across all 15 mode pairs.
The largest gain is seen in \texttt{magnitude+rcm\_log\_density}, where accuracy improves by 1.13\,pp and F1 by 0.79\,pp; the smallest
is on \texttt{magnitude+log\_density}, where accuracy stays the same and only F1 increases by 0.58\,pp.
No pair degrades at higher resolution.

The cost of the higher resolution is an $4\times$ increase in inference time, as the dual-channel 64\,px model runs at approximately 2.8\,ms/sample and the
...
In the single-channel experiments, results were more mixed: \texttt{binary\_128} and \texttt{density\_128} outperformed
their 64\,px counterparts, but \texttt{magnitude\_64} beat \texttt{magnitude\_128} in accuracy and F1.
...
\begin{table}[H]
  \centering
  \caption{Campaign~5 - Ensemble results: softmax-averaging of four independently trained dual-channel models.
...
%%TODO: Ensemble Acc/F1 vs. best individual model in the same configuration.
%%      How many models are needed before returns diminish?
%%      Failure rate of ensemble vs. individual.
%
\textbf{Campaign~3 ensemble.}

The Campaign~3 ensemble —
\texttt{magnitude+signed\_magnitude}, \texttt{magnitude+rcm\_signed\_magnitude},
\texttt{magnitude+symmetry}, and \texttt{rcm\_magnitude+signed\_magnitude} (all 64\,px) —
...
by $+1.34$\,pp in accuracy and $+1.05$\,pp in F1.
...
\textbf{Campaign~4 ensemble.}
The Campaign~4 ensemble —
\texttt{magnitude+log\_density}, \texttt{magnitude+rcm\_log\_density},
\texttt{rcm\_magnitude+rcm\_signed\_magnitude}, and \texttt{symmetry+log\_density} (all 128\,px) —
achieves 67.29\% accuracy and 66.77\% F1, compared to the best individual member
(\texttt{magnitude+rcm\_log\_density\_128} at 66.56\% accuracy and 65.53\% F1),
a gain of $+0.73$\,pp in accuracy and $+1.24$\,pp in F1.
...
The replication even exceeds the paper in accuracy by 3.43\,pp, as seen in \autoref{tab:comparison-mm}, and
...
\begin{table}[H]
  ...
  \caption{Comparison against MM-AutoSolver \cite{xiong2025mmautosolver}.
           Best single: \texttt{magnitude+rcm\_log\_density} at 128\,px (Campaign~4).
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    This thesis (best single, C4)      & 66.56 & 69.15 & 68.88 & 65.53 \\
...
First, the class distribution in the dataset differs substantially from the MM-AutoSolver dataset, which uses a dataset
where the \texttt{fbcgsr+jacobi} and \texttt{bcgsl+none} classes make up more than 36\% of the whole distribution.
...
within FreeFEM and OpenFOAM.
Within the thesis, only SuiteSparse matrices with real-world applications are used; the remainder consists of synthetically generated ones.

Despite the lower accuracy, the macro-averaged metrics are less sensitive to class distribution and can still be compared.
The thesis model even achieves around 7\,pp better results in Macro Precision and Macro Recall, meaning that despite the harder
...
%TODO: F1 / failure rate per solver-preconditioner class.
%      Which solvers are easiest / hardest to predict?
%      Correlation with class size (rare classes harder).
%      Insight for framework integration: which solver pairs need more data.
...
\begin{table}[H]
  \centering
  \caption{Per-class results for the Campaign~4 ensemble (4 models: magnitude+log\_density,
...
\texttt{cg+ilu} stands out with a 30.5\% failure rate.
This is likely a side-effect of the convergence penalty interacting with the new ILU-related
features (\texttt{min\_diag\_dominance}, \texttt{diag\_dom\_variance}): the penalty reduces
failure rates globally but appears to shift prediction errors for borderline cases where the
new features signal marginal ILU suitability.
\texttt{symmlq+sor} (15.4\%) and \texttt{cr+ilu} (6.5\%) also carry elevated failure rates,
as both rely on preconditioners sensitive to spectral properties not fully captured by the
feature set.
Ten classes achieve 0\% failure rate: \texttt{minres+gamg}, \texttt{cr+eisenstat},
\texttt{gmres+gamg}, \texttt{cr+jacobi}, \texttt{cg+eisenstat}, \texttt{bcgsl+none},
\texttt{fbcgsr+ilu}, \texttt{dgmres+none}, \texttt{cgs+gamg}, and \texttt{fgmres+gamg}.

\section{Practical Inference Cost}\label{sec:inference-cost}

%TODO: Latency: ms/sample on CPU for small, large, and dual-channel models.
%      Mean runtime ratio (predicted time / oracle best time).
%      Is the prediction overhead negligible compared to actual solve time?

Model inference is fast enough to be neglectable compared to actual solver runtimes.
\autoref{tab:inference-latency} summarises the per-sample cpu latency for the main model configurations, measured on the
validation set of 969 samples in the evaluation set.
