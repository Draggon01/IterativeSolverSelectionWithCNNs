...
\paragraph[Inference cost.]{Inference cost.}
...
In this thesis, only SuiteSparse matrices with real-world applications are used; the remainder consists of synthetically generated ones.
...
features \texttt{min\_diag\_\allowbreak dominance} and \texttt{diag\_dom\_variance}.
The penalty reduces failure rates globally but appears to shift prediction errors for borderline
cases where the new features signal marginal ILU suitability.
...
feature set.
Ten classes achieve 0\% failure rate:
\texttt{minres+gamg}, \texttt{cr+eisenstat}, \texttt{gmres+gamg}, \texttt{cr+jacobi},
\texttt{cg+eisenstat}, \texttt{bcgsl+none}, \texttt{fbcgsr+ilu}, \texttt{dgmres+none},
\texttt{cgs+gamg}, and \texttt{fgmres+gamg}.
...
The Campaign~4 ensemble is the most expensive configuration at approximately 46.5\,ms per sample, but even this remains well below typical iterative solver runtimes, which commonly run in the range of seconds for the matrix sizes considered in this thesis.
Prediction overhead is therefore negligible in all configurations.
