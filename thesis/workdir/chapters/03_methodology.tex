...
which solver-preconditioner classes are used, which features are selected and why and also describes the architecture used inside the PyTorch model.
...
The dataset is constructed by combining SuiteSparse matrices with synthetically generated matrices and saves the information needed for
further processing into an HDF5 file.
For the dataset a cap of winning solver-preconditioner pairs is introduced to counter a certain bias towards overrepresented pairs in the training phase.
...
the actual winner is always determined empirically by benchmarking~\cite{saad2003iterative}.
...
A more systematic bucket design falls outside the scope of this thesis and these downsides are therefore accepted as a limitation.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}llp{4.5cm}@{}}
    ...
    \midrule
    \multicolumn{3}{@{}l@{}}{\textit{Nonsymmetric}} \\[2pt]
...
Some solvers do not work, or only work, on symmetric matrices, therefore they are not run on these specific ones.
A pair is valid and gets saved with its spent time if the result is within a certain tolerance.
The label a matrix gets in the end is only the fastest solver-preconditioner pair, the so-called winner.

For each matrix the matrix itself, the features and all the pairs with solving time or an entry for non-convergence are stored.
...
Since they are downloaded and stored as .mtx files, the storage of the full matrices in the HDF5 dataset file does not make sense
and is therefore omitted to save space.
A separate folder for the mtx files is used as a cache which reduces storage requirements while keeping the dataset portable.
...
\begin{table}[H]
  \centering
  \caption{17 numerical features used in MM-AutoSolver~\cite{xiong2025mmautosolver} (Table~2).}
...
Feature 16 distinguishes structurally symmetric from truly asymmetric matrices, helping to separate \texttt{gmres} from \texttt{fgmres} predictions.
...
\begin{table}[H]
  \centering
  \caption{The 6 additional matrix features introduced in Campaign~2, expanding the feature vector from 14 to 20 entries.}
...
\begin{table}[h]
  \centering
  \caption{The 14 image modes evaluated in the single-channel experiments. The six \texttt{rcm\_*} variants apply Reverse Cuthill--McKee reordering to the matrix before rendering with the corresponding base mode.}
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    \texttt{rcm\_binary}           & \multirow{6}{*}{\parbox{7cm}{RCM-reordered variants of the above modes. Reverse Cuthill--McKee reordering reduces matrix bandwidth before rendering, grouping structurally related entries closer to the diagonal.}} \\
    \texttt{rcm\_density}          & \\
...
Also two different model sizes are introduced to evaluate the effect of capacity.
The CNN branch supports a single image channel and a dual one where 2 different images can be used as input.
...
$c$ output channels, followed by batch normalisation and ReLU activation, and a
...
and a skip connection: $\mathbf{y} = \text{ReLU}(\mathbf{x} + \text{block}(\mathbf{x}))$.
...
(large model).
No dropout is applied in this branch.
The features are already low-dimensional and the risk of overfitting here is low.
...
\]
and passed through a two-layer classification head with ReLU activation and dropout,
...
A cosine annealing schedule reduces the learning rate to zero over the full
...
Checkpoints are saved every 5 epochs and the final epoch checkpoint is used for evaluation.